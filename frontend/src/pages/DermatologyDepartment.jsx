...
// ═════════════════════════════════════════════════════════════════════════════
const DermatologyDepartment = ({ onBack }) => {
  ...
  const [newDoctor, setNewDoctor] = useState({
    name: '', degree: '', department: 'Dermatology',
    experience: '', status: 'active', shift: 'first',
  });

  // Equipment (Dermatology-specific)
  const equipment = [
    { id:1, name:'Dermatoscope',    room:'Room 501', status:'available' },
    { id:2, name:'Laser Machine',   room:'Room 502', status:'in-use'   },
    { id:3, name:'Cryotherapy Unit',room:'Room 503', status:'available' },
    { id:4, name:'UV Light Therapy',room:'Room 504', status:'available' },
  ];
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Filter appointments for Dermatology only
      const allAppts  = apptRes.appointments ?? [];
      const deptAppts = allAppts.filter(
        a => a.department?.toLowerCase() === 'dermatology'
      );

      // Dermatology doctors only
      const deptDoctors = doctorsRes.doctors_by_department?.['Dermatology'] ?? [];
      ...
      const deptStat = statsRes.dept_stats?.['Dermatology'];
      setStats({
  ...
  const handleCompleteCheckup = async patientId => {
    try {
      await api.put(`/appointments/${patientId}`, {
        appointment_type: 'Routine', problem_text: '', department: 'Dermatology', status: 'completed',
      });
    } catch {}
  ...
  const handleAddDoctor = async e => {
    ...
    setNewDoctor({ name:'', degree:'', department:'Dermatology', experience:'', status:'active', shift:'first' });
  };
    ...
    <div className="cardiology-container">
      ...
      <div className="cardio-header">
        ...
        <div className="header-center">
          <h1 className="dept-heading">Dermatology Department</h1>
        </div>
      ...
      {loading && (
        <div style={{ display:'flex', justifyContent:'center', alignItems:'center', minHeight:300, flexDirection:'column', gap:12, color:'#6b7280' }}>
          ...
          Loading Dermatology data from database…
        </div>
      )}
          ...
          <div style={{ margin:'16px 28px 0', borderRadius:14, overflow:'hidden', border:'1.5px solid #7c5cbf', boxShadow:'0 4px 20px rgba(124,92,191,.15)' }}>
            <div style={{ background:'linear-gradient(135deg,#7c5cbf,#1a1d2e)', padding:'13px 18px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div style={{ color:'#fff', fontWeight:700, fontSize:14, display:'flex', alignItems:'center', gap:8 }}>
                🤖 AI Operations Assistant — Dermatology
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <select value={editingAppointment.department} onChange={e => setEditingAppointment({...editingAppointment,department:e.target.value})}>
                      {['Dermatology','Cardiology','Neurology','Orthopedics','Pediatrics','General Medicine'].map(d => <option key={d}>{d}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
      ...
      {showAllAppointments && (
        <div className="modal-overlay" onClick={() => setShowAllAppointments(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>All Appointments — Dermatology</h2>
        ...
        <div className="modal-overlay" onClick={() => setShowAddDoctorForm(false)}>
          <div className="modal-content small" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddDoctor} className="add-doctor-form">
              ...
              <div className="form-group">
                <label>Department *</label>
                <select value="Dermatology" disabled><option>Dermatology</option></select>
              </div>
              <div className="form-group">
