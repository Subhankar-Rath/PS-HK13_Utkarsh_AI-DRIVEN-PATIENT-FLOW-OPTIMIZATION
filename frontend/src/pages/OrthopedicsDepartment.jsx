...
// ═════════════════════════════════════════════════════════════════════════════
const OrthopedicsDepartment = ({ onBack }) => {
  ...
  const [newDoctor, setNewDoctor] = useState({
    name: '', degree: '', department: 'Orthopedics',
    experience: '', status: 'active', shift: 'first',
  });

  // Equipment (Orthopedics-specific)
  const equipment = [
    { id: 1, name: 'X-Ray Machine',     room: 'Room 301', status: 'available' },
    { id: 2, name: 'MRI Scanner',        room: 'Room 302', status: 'in-use'   },
    { id: 3, name: 'Ultrasound Unit',    room: 'Room 303', status: 'available' },
    { id: 4, name: 'CT Scanner',         room: 'Room 304', status: 'available' },
    { id: 5, name: 'Traction Equipment', room: 'Room 305', status: 'available' },
    { id: 6, name: 'Arthroscope',        room: 'Room 306', status: 'in-use'   },
  ];
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Filter appointments for Orthopedics only
      const allAppts  = apptRes.appointments ?? [];
      const deptAppts = allAppts.filter(
        a => a.department?.toLowerCase() === 'orthopedics'
      );

      // Orthopedics doctors only
      const deptDoctors = doctorsRes.doctors_by_department?.['Orthopedics'] ?? [];

      // ── Map doctors to UI shape ──
      ...
      // ── Redistribute patients across active doctors ──
      ...
      const deptStat = statsRes.dept_stats?.['Orthopedics'];
      setStats({
  ...
  const toggleDoctorStatus = async doctorId => {
    ...
    if (newStatus === 'inactive') {
      ...
      await triggerAI('doctor_inactive', {
        ...
        specialNote:  'Orthopedic cases may require specialized triage for mobility-impaired patients.',
      });
    }
  };
  ...
  const handleCompleteCheckup = async patientId => {
    try {
      await api.put(`/appointments/${patientId}`, {
        appointment_type: 'Routine', problem_text: '', department: 'Orthopedics', status: 'completed',
      });
    } catch {}
  ...
  const handleSaveAppointment = async updated => {
    ...
    if (needsReason && changeReason.trim()) {
      ...
      // 🤖 Orthopedics-specific AI context
      ...
      await triggerAI(aiType, {
        ...
        department:      'Orthopedics',
        specialNote:     'Consider mobility aids and physical therapy implications for rescheduled orthopedic patients.',
      });
    }
  ...
  const handleAddDoctor = async e => {
    ...
    setNewDoctor({ name: '', degree: '', department: 'Orthopedics', experience: '', status: 'active', shift: 'first' });
  };
  ...
  const triggerEmergencyAI = async (doctor, patient) => {
    ...
    await triggerAI('emergency_reassignment', {
      ...
      specialNote:        'For orthopedic emergencies, assess fractures, dislocations, or spinal injuries for immediate imaging.',
    });
  };
    ...
    <div className="cardiology-container">
      ...
      <div className="cardio-header">
        ...
        <div className="header-center">
          <h1 className="dept-heading">Orthopedics Department</h1>
        </div>
      ...
      {loading && (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 300, flexDirection: 'column', gap: 12, color: '#6b7280' }}>
          ...
          Loading Orthopedics data from database…
        </div>
      )}
          ...
          <div style={{ margin: '16px 28px 0', borderRadius: 14, overflow: 'hidden', border: '1.5px solid #7c5cbf', boxShadow: '0 4px 20px rgba(124,92,191,.15)' }}>
            <div style={{ background: 'linear-gradient(135deg,#7c5cbf,#1a1d2e)', padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ color: '#fff', fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                🤖 AI Operations Assistant — Orthopedics
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <select value={editingAppointment.department} onChange={e => setEditingAppointment({ ...editingAppointment, department: e.target.value })}>
                      {['Orthopedics', 'Cardiology', 'Neurology', 'Dermatology', 'Pediatrics', 'General Medicine'].map(d => <option key={d}>{d}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
      ...
      {showAllAppointments && (
        <div className="modal-overlay" onClick={() => setShowAllAppointments(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>All Appointments — Orthopedics</h2>
        ...
        <div className="modal-overlay" onClick={() => setShowAddDoctorForm(false)}>
          <div className="modal-content small" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddDoctor} className="add-doctor-form">
              ...
              <div className="form-group">
                <label>Department *</label>
                <select value="Orthopedics" disabled><option>Orthopedics</option></select>
              </div>
              <div className="form-group">
...
export default OrthopedicsDepartment;